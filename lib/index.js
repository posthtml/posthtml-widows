export default (options = {}) => tree => {
  ...
  options.ignore = options.ignore || [{ start: '{{', end: '}}' }]
  ...
  const process = (node, shouldReplace = false) => {
    ...
    if (typeof node === 'string') {
      ...
      // Check if the string contains any of the ignore patterns
      for (const pattern of options.ignore) {
        const ignorePattern = new RegExp(`${pattern.start}.*?${pattern.end}`, 'g')
        if (ignorePattern.test(node)) {
          return node
        }
      }
